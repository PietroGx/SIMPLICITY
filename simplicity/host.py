...
class Host:
    ...
    def plot_intra_host(self,time,step):
        '''
        ...
        for state in states:
            plt.plot(t,self._data_plot_model(state,time,step), color = colormap(normalize(state)))
    ...
    def plot_comparison(self):
        ...
        normal and immunocompromised individuals
        ...
        plt.plot(t,self._data_plot_model(0,50,.1), color = 'orange', 
                 label = 'normal')
        
        # setup intra-host model matrix and calculate matrix exponential
        self.A = self._matrix(133.5)
        self.A_ex = scipy.linalg.expm(self.A)
        
        t = np.arange(0,300,1)
        plt.plot(t,self._data_plot_model(0,300,1), color = 'blue', 
